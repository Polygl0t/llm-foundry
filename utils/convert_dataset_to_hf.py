        ...
        for name, feature in ds.features.items():
            if isinstance(feature, datasets.features.Sequence):
                ...
                if isinstance(inner_feature, dict):
                    features_info[name] = {"type": "list_of_dicts", "fields": {}}
        ...
        if n_chunks == 1:
            ...
            for chunk_idx, chunk_indices in enumerate(indices):
                ...
                output_file = os.path.join(
                    output_dir, f"train-{file_start_idx + chunk_idx:05d}.parquet"
                )
...
def process_folder(
    ...
    for file_num, input_file in enumerate(data_files, 1):
        ...
        if results and results[0]["success"]:
            ...
            tqdm.write(
                f"  ✓ Done: {chunks_created} chunk(s), {output_size_gb:.2f} GB output, {len(data_files) - file_num} files remaining"
            )
    ...
    if total_output_files > 0:
        ...
        for i in range(total_output_files):
            ...
            new_name = os.path.join(
                output_dir, f"train-{i:05d}-of-{total_output_files:05d}.parquet"
            )
...
def generate_yaml_config(folder_stats, output_path, default_dataset_name):
    ...
    for stats in folder_stats:
        ...
        yaml_config += (
            f"  download_size: {stats['size_bytes']}\n  dataset_size: {stats['size_bytes']}\n"
        )
    ...
    token_counts = {
        s["folder_name"]: s["token_count"]
        for s in folder_stats
        if s and s.get("token_count", 0) > 0
    }
    if token_counts:
    ...
    yaml_config += (
        f"| **Total** | **{total_files}** | **{total_rows:,}** | **{total_size_gb:.2f} GB** |\n"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the directory containing data folders",
    )
    parser.add_argument(
        "--dataset_type",
        type=str,
        ...
        default="jsonl",
        help="Type of source files (jsonl or parquet)",
    )
    parser.add_argument(
        "--output_path", type=str, required=True, help="Path to save the converted dataset"
    )
    parser.add_argument(
        "--cache_dir", type=str, default="./.cache", help="Path to cache directory for datasets"
    )
    parser.add_argument(
        "--max_chunk_size_gb",
        type=float,
        default=5.0,
        help="Maximum output file size in GB. Default: 5.0",
    )
    parser.add_argument(
        "--compression_ratio",
        type=float,
        default=0.25,
        help="Expected JSONL to Parquet compression ratio. Default: 0.25",
    )
    parser.add_argument(
        "--default_dataset_name",
        type=str,
        default="default",
        help="Default config name for the dataset",
    )
    parser.add_argument(
        "--num_workers", type=int, default=8, help="Number of parallel workers for upload"
    )
    parser.add_argument(
        "--new_repo_id", type=str, default=None, help="HuggingFace repository ID for upload"
    )
