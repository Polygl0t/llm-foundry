...
def inspect_model(
    ...
    print(
        f"    Architecture:             {config.architectures[0] if config.architectures else 'Unknown'}"
    )
    ...
    print(
        f"    Tie word embeddings:      {config.tie_word_embeddings if hasattr(config, 'tie_word_embeddings') else 'N/A'}"
    )
