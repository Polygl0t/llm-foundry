...
def main(token, repo_id, root_dir):
    ...
    try:
        ...
        step_folders = [
            ...
            and not os.path.islink(os.path.join(root_dir, f))
        ]
        ...
        for step in step_folders:
            ...
            try:
                if step == "main":
                    ...
                    api.upload_folder(
                        folder_path=step_folder_path, repo_id=repo_id, revision="main"
                    )
                    ...
                    api.upload_folder(folder_path=step_folder_path, repo_id=repo_id, revision=step)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--repo_id", type=str, required=True, help="Repository ID (e.g., username/repo_name)"
    )
    parser.add_argument(
        "--root_dir", type=str, required=True, help="Root directory containing step folders"
    )
