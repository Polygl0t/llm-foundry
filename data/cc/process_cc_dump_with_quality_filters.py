...
from utils import get_logger, initialize_or_load_metadata, write_metadata
...
def main(args):
    ...
    LANG_FILTER_MAPPING = {"pt": "por_Latn", "bn": "ben", "hi": "hin", "de": "deu"}
    ...
    for lang in os.listdir(WARC_EXTRACTION_OUTPUT):
        ...
        with open(LANG_CONFIG_MAPPING[lang]) as f:
        ...
        try:
            ...
            logger.warning(
                "Error writing to %s with ensure_ascii=False: %s",
        ...
        # Update metadata
        updated_metadata = {"lines": total_documents, "tokens": total_tokens}
    ...
    if language_statistics:
        # Calculate totals
        total_all_documents = sum(
            stats["total_documents"] for stats in language_statistics.values()
        )
        ...
        for lang, stats in sorted(language_statistics.items()):
            avg_tokens = (
                stats["total_tokens"] / stats["total_documents"]
        ...
        logger.info(
            "%s %15d %18d %18.2f", f"{'TOTAL':<12}", total_all_documents, total_all_tokens, avg_all
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The folder containing language configuration files",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Limit the number of WARC files to process (useful for debugging)",
    )
    parser.add_argument("--logs_folder", type=str, default="./logs", help="Folder to store logs")
    parser.add_argument(
        "--warc_extraction_output",
        type=str,
        default="./warc_extraction",
        help="Folder to store WARC extraction output",
    )
    parser.add_argument(
        "--quality_filter_output",
        type=str,
        default="./quality_filter",
        help="Folder to store quality filter output",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Tokenizer name or path (default: Qwen/Qwen3-0.6B)",
    )
    parser.add_argument(
        "--languages",
        nargs="+",
        type=str,
        default=None,
        help="List of languages to filter (e.g., --languages bn pt hi)",
    )
