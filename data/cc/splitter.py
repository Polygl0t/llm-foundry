...
def _chunk_writer(directory, unique_hash):
    """Yield (chunk_path, file_handle) pairs for sequential chunk writing."""
    chunk_idx = 0
    while True:
...
def split_jsonl_file(
    file_path: str, max_tokens_per_chunk: int = 100_000_000, size_threshold_gb: float = 1.0
):
    ...
    writer = _chunk_writer(directory, unique_hash)
    chunk_path, chunk_file = next(writer)
    current_chunk_tokens = 0
    chunks_created = [chunk_path]
        ...
        with open(file_path, encoding="utf-8", errors="replace") as infile:
            for line in infile:
                ...
                try:
                    ...
                    if (
                        current_chunk_tokens > 0
                        and current_chunk_tokens + token_count > max_tokens_per_chunk
                    ):
                        chunk_file.close()
