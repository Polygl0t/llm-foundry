"""
LLM-as-a-Judge Dataset Quality Filter

Evaluates and filters text samples using an LLM judge (powered by vLLM) to assess
quality, relevance, or custom criteria. Ideal for large-scale dataset curation.

Usage:
    # Basic filtering with judge model
    python llm_filter.py --model_name Qwen/Qwen2.5-14B-Instruct \
        --dataset_path data/ --column_name text --output_dir filtered/ \
        --system "Rate text quality from 1-10" \
        --prompt_prefix "Text to evaluate: " \
        --prompt_suffix "\n\nProvide score and reason in JSON format."
    
    # Multi-GPU with chunking for long documents
    python llm_filter.py --model_name meta-llama/Llama-3.1-70B-Instruct \
        --tensor_parallel_size 4 --gpu_memory_utilization 0.95 \
        --dataset_path dataset.jsonl --column_name content \
        --max_chunk_size 4096 --chunk_once --output_dir results/
"""
...
def parse_filter_response(sample):
    """Parse and extract score and reason from generated filter response"""
    try:
        json_string = json_repair.repair_json(sample)
        decoded_object = json_repair.loads(json_string)
        score = decoded_object.get("score", None)
        reason = decoded_object.get("reason", None)
        return score, reason
    except Exception as e:
        print(f"[ERROR] Failed to parse response: {e}")
        return None, None

def save_samples(text, samples, subset, source, output_file, idx):
    """Save generated filter samples with scores and reasons to a file."""
    with open(output_file, "a", encoding="utf-8") as f:
        for sample in samples:
            score, reason = parse_filter_response(sample)
...
def process_sample(sample, counter, args, model, tokenizer, sampling_params, file_path):
    ...
    for i, text in enumerate(text_samples):
        ...
        print(f"[EVALUATING] Sample {counter}. Chunk {i + 1}/{len(text_samples)}...")
        ...
        subset = sample.get('task_type') or sample.get('subset')
        source = sample.get('token_count', None)
        idx = sample.get('id', f"row_{counter}")
...
def main(args):
    ...
    print(f"[INFO] Judge: {args.model_name}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Filter text samples using a Hugging Face model and vLLM.")
    ...
    parser.add_argument("--column_name", type=str, required=True, help="Column in the dataset where the text is located.")
    parser.add_argument("--output_dir", type=str, required=True, help="Directory to save the filtered samples.")
    ...
    parser.add_argument("--row_start", type=int, default=None, help="Row index to start filtering samples.")
