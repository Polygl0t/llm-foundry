...
def create_language_filter(languages):
    ...
    if not target_codes:
        raise ValueError(
            f"No valid languages specified. Available languages: {list(LANGUAGE_CODES.keys())}"
        )
...
def main(args):
    ...
    assert args.input_type in ["jsonl", "parquet"], (
        "Dataset type must be either 'jsonl' or 'parquet'."
    )
    ...
    logger.info(
        f"Loaded dataset with {len(dataset):,} examples from {args.input_type.upper()} files."
    )
    ...
    if args.text_column not in dataset.column_names:
        raise ValueError(
            f"Column '{args.text_column}' not found in dataset. Available columns: {dataset.column_names}"
        )
    ...
    if "token_count" in filtered_dataset.column_names:
        ...
        removed_tokens_percentage = (
            (removed_tokens / original_tokens * 100) if original_tokens > 0 else 0
        )
        logger.info(f"Original tokens: {original_tokens:,}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_dir", type=str, required=True, help="Directory containing the input dataset files"
    )
    parser.add_argument(
        "--output_dir", type=str, required=True, help="Output directory to save filtered dataset"
    )
    parser.add_argument(
        "--languages",
        type=str,
    ...
    parser.add_argument(
        "--text_column", type=str, default="text", help="Name of the text column in the dataset"
    )
    ...
    parser.add_argument(
        "--save_excluded",
        action="store_true",
        help="Save excluded samples instead of included ones (for testing)",
    )
