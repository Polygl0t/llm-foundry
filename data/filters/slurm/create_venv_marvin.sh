...
#
# This is the same script as in distributed/slurm/create_venv_marvin.sh,
# but installs the additional packages for training annotators `evaluate` and `sklearn`.
#
# Usage:
#   sbatch data/filters/slurm/create_venv_marvin.sh
#
#############################################
...
echo "===== Installing additional packages for annotation pipeline ====="
uv pip install -e evaluate==0.4.6 scikit-learn==1.9.0 \
...
packages = [
    ...
    "codecarbon", "trackio", "evaluate", "scikit-learn",
]
for pkg in packages:
    try:
        ver = version(pkg)
