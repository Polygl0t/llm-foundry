"""
Fully Sharded Data Parallel (FSDP) Training Script with AdamW Optimizer

This script implements distributed training for large language models using PyTorch FSDP,
supporting various decoder architectures (Llama, SmolLM3, Gemma3, Qwen2/3).
...
def main(specs, slurm_job_id, hardware):
    ...
    if args.resume_from_checkpoint:
        ...
        checkpoint = torch.load(checkpoint, mmap=True, map_location=torch.device('cpu'), weights_only=False)
        set_optimizer(
            model=model,
    ...
    for completed_steps in range(1, max_steps + 1):
        ...
        if master_process:
            ...
            logger.info(f"Training | step: {completed_steps:5d} | loss: {accumulated_loss.item():.6f} | adam-lr: {lr:.4e} | lr stage: '{current_lr_stage}' | norm: {float(norm):.4f} | dt: {dt*1000:.2f}ms | tok/sec: {tokens_per_sec:.2f} | VRAM: {used_vram:.2f} | MFU: {mfu:.2f}%")

            with open(log_file, "a") as f:
                f.write(f"Training | step: {completed_steps:5d} | loss: {accumulated_loss.item():.6f} | adam-lr: {lr:.4e} | lr stage: '{current_lr_stage}' | norm: {float(norm):.4f} | dt: {dt*1000:.2f}ms | tok/sec: {tokens_per_sec:.2f} | VRAM: {used_vram:.2f} | MFU: {mfu:.2f}%\n")
