"""
Distributed Data Parallel (DDP) Training with Adam+Muon Optimizer

Production-ready training script for transformer-based causal language models using
PyTorch DDP with hybrid Adam+Muon optimization. Designed for multi-GPU, multi-node clusters.
...
def main(specs, slurm_job_id, hardware):
    ...
    for completed_steps in range(1, max_steps + 1):
        ...
        if master_process:
            ...
            logger.info(f"Training | step: {completed_steps:5d} | loss: {accumulated_loss.item():.6f} | adam-lr: {adam_lr:.4e}  | muon-lr: {muon_lr:.4e} | lr stage: '{current_lr_stage}' | norm: {norm:.4f} | dt: {dt*1000:.2f}ms | tok/sec: {tokens_per_sec:.2f} | VRAM: {used_vram:.2f} | MFU: {mfu:.2f}%")

            with open(log_file, "a") as f:
                f.write(f"Training | step: {completed_steps:5d} | loss: {accumulated_loss.item():.6f} | adam-lr: {adam_lr:.4e}  | muon-lr: {muon_lr:.4e} | lr stage: '{current_lr_stage}' | norm: {norm:.4f} | dt: {dt*1000:.2f}ms | tok/sec: {tokens_per_sec:.2f} | VRAM: {used_vram:.2f} | MFU: {mfu:.2f}%\n")
            if args.wandb_token is not None:
