...
Note: This script support resume capability via checkpoints. If the process is interrupted.
    ...
    async def simple_rollout(
        ...
        if isinstance(document.text, list) and all(isinstance(msg, dict) for msg in document.text):
            ...
            if len(content) > char_budget:
                if chunk:
                    # Truncate the document so it fits within the budget.
                    logger.info(
                        f"Truncating document: content length {len(content)} chars exceeds "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--chunk", action="store_true", help="Truncate overflowing docs instead of skipping them"
    )
