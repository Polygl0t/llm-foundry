"""
Batch Translation Pipeline with vLLM

This script translates text samples from English to Portuguese using Hugging Face models and vLLM,
optimized for "hard to translate" content such as technical tutorials, code snippets, and mixed-language text.

Output Format:
    {"idx": "row_0", "original_text": "Hello", "en": "Hello", "pt": "Olá"}

Example usage:
    python generate_translate.py \
        --model_name meta-llama/Llama-3.1-8B-Instruct \
        --dataset_path data/english_corpus.jsonl \
        --column_name text \
        --output_dir outputs/translations \
...
def parse_and_save_sample(sample, file_prefix, original_text, output_file):
    """Parse a single sample and save it to file"""
    try:
        ...
        json_data = {
            ...
            "en": decoded_object.get("en"),
            "pt": decoded_object.get("pt"),
        }
    except Exception:
        print(f"[ERROR] Could not extract JSON from text: {file_prefix}")
        json_data = {
            "idx": file_prefix,
            "original_text": original_text,
            "en": None,
...
def save_samples(samples, output_file, file_prefix, original_text):
    """Save generated samples to a file."""
    for sample in samples:
        parse_and_save_sample(sample, file_prefix, original_text, output_file)
...
def process_sample(sample, counter, args, model, tokenizer, sampling_params, file_path):
    ...
    for i, text in enumerate(text_samples):
        ...
        save_samples(generated_samples, file_path, file_prefix, text)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Translate test samples using a Hugging Face models and vLLM.")
    ...
    parser.add_argument("--system", type=str, default=None, help="System message to prepend to the input.")
